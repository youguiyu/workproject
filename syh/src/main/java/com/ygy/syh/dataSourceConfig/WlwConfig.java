package com.ygy.syh.dataSourceConfig;
 ...
 */
@Configuration
@MapperScan(basePackages = WlwConfig.PACKAGE,sqlSessionFactoryRef = "wlwSqlSessionFactory")
public class WlwConfig {

    public static final String PACKAGE = "com.ygy.syh.dao.wlw";

    public static final String MAPPER_LOCATION = "classpath:mapper/wlw/*.xml";
...
//    cluster.datasource.password = root
    ...
    @Value("${wlw.datasource.driver-class-name}")
    private String driverClassName;

    @Value("${wlw.datasource.url}")
    private String url;

    @Value("${wlw.datasource.username}")
    private String username;

    @Value("${wlw.datasource.password}")
    private String password;

    //初始化数据库连接
    @Bean(name="wlwDataSource")
    ...
    //数据源事务管理器
    @Bean(name="wlwDataSourceTransactionManager")
    ...
    public SqlSessionFactory clusterSqlSessionFactory(@Qualifier("wlwDataSource") DataSource clusterDataSource) throws Exception{
        final SqlSessionFactoryBean sqlSessionFactoryBean = new SqlSessionFactoryBean();
        sqlSessionFactoryBean.setDataSource(clusterDataSource);
        Resource[] resource = new PathMatchingResourcePatternResolver().getResources(WlwConfig.MAPPER_LOCATION);
